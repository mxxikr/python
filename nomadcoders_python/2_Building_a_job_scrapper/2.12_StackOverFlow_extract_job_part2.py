...
def extract_job(div_job):
    ...
    company, location = div_job.find("h3").find_all("span", recursive=False) # 전부 가져오는 것 방지 -> 첫번째 단계의 span만 가져올 수 있음
    # company = company_row[0] # 리스트 안에 두개의 item이 있는걸 알기 때문에 unpacking vaule 사용 가능
    # location = company_row[1] # 리스트의 첫번째 요소가 company, 두번째 요소가 location
